...
class LiquiGoals_PleesherImplementation extends PleesherImplementation {
	...
	public function getUserPageData( User $user ) {
		...
		$user_professions = array_map( function($user_profession) use($user) {
			...
			$user_profession->kudos_needed_for_next_level = isset( $user_profession->levels_kudos[ $user_profession->level ] ) ? $user_profession->levels_kudos[ $user_profession->level ] : $user_profession->kudos;
